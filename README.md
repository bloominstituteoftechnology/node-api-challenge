...
- [x] Mention two parts of Express that you learned about this week.
    ...
    Express provides routing. A way to select which request handler function is executed based on the URL and which HTTP Method was used. 

    Convenience helpers. Provide functionality out of the box. Provides extension methods added to the response and request methods, such as, request.ip, and reponse.redirect. 

    Views, which allow dynamically rendering HTML on the server and also generate it using other languages. 

- [x] Describe Middleware?

    Functions that get the rquest from reponse, can perform operations on them, and can either move into the next middleware, or return a aspeonse back to the client. 
...
- [x] What can the API return to help clients know if a request was successful?
    ...
    Http response codes, with an error number, may be returned. 

- [x] How can we partition our application into sub-applications?

    We can partition out functionality into other files and use the require keyword to import them. 
