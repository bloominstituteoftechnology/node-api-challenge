...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      error: "error posting an project, type in a name and description",
    });
  }
});
  ...
  projectDb
    ...
    .then((result) => {
      if (result) {
        ...
        res.status(404).json({
          message:
            "could not find this project, this project id does not exist",
        });
      }
    })
    .catch((err) => {
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res.json({ status: `User Id: ${result} has been deleted` });
  } catch {
